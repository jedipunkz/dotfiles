---
name: zellij-swarm
description: Zellij の pane 管理を使って複数の Codex エージェントを並列起動し、タスクを分散実行するオーケストレータースキル。「swarm で」「並列エージェントで」「zellij swarm」「エージェントを N 個起動」「複数エージェントに分けて」などのリクエストがあった場合に使用します。
---

# Zellij Swarm Skill

Zellij の pane 管理と git worktree を組み合わせ、複数の Codex エージェントが
独立したタスクを並列実行するワークフローです。

## 前提条件

- Zellij セッション内で Codex を起動していること (`$ZELLIJ` 環境変数が存在)
- git リポジトリ内で実行していること
...
```
.agents/skills/zellij-swarm/
├── SKILL.md                    # このファイル
├── templates/
│   ├── AGENTS.md.template      # エージェント用 AGENTS.md テンプレート
│   └── codex-task.md.template  # タスク定義テンプレート
└── scripts/
...
```bash
.agents/skills/zellij-swarm/scripts/setup-worktrees.sh agent-1 agent-2 agent-3
```
...
**`.gitworktree/<agent>/AGENTS.md`**:
`templates/AGENTS.md.template` の内容を Read tool で読み取り、Write tool で書き込む。

**`.gitworktree/<agent>/.codex-task.md`**:
`templates/codex-task.md.template` を参考に、具体的なタスク内容を記述して Write tool で書き込む。
...
```bash
.agents/skills/zellij-swarm/scripts/launch-panes.sh agent-1 agent-2 agent-3
```

各ペインは `.swarm-start.sh` を経由して `.codex-task.md` の内容を初期プロンプトとして Codex に渡す。
...
```bash
.agents/skills/zellij-swarm/scripts/monitor.sh agent-1 agent-2 agent-3
```
...
```bash
.agents/skills/zellij-swarm/scripts/merge.sh agent-1 agent-2 agent-3
```
...
```bash
.agents/skills/zellij-swarm/scripts/cleanup.sh agent-1 agent-2 agent-3
```

途中で中断した場合や worktree が壊れた場合は `--force` オプションを使用:

```bash
.agents/skills/zellij-swarm/scripts/cleanup.sh --force agent-1 agent-2 agent-3
```
...
.agents/skills/zellij-swarm/scripts/setup-worktrees.sh agent-a agent-b

# 2. タスクファイルを Write tool で書き込む (agent-a, agent-b それぞれ)

# 3. Phase 1 のペイン起動
.agents/skills/zellij-swarm/scripts/launch-panes.sh agent-a agent-b

# 4. Phase 1 完了まで待機 → 自動で merge + cleanup
.agents/skills/zellij-swarm/scripts/run-phase.sh agent-a agent-b
```
...
.agents/skills/zellij-swarm/scripts/setup-worktrees.sh agent-c

# 6. タスクファイルを Write tool で書き込む (agent-c)

# 7. Phase 2 のペイン起動
.agents/skills/zellij-swarm/scripts/launch-panes.sh agent-c

# 8. Phase 2 完了まで待機 → 自動で merge + cleanup
.agents/skills/zellij-swarm/scripts/run-phase.sh agent-c
```
...
5. **`--ask-for-approval never --sandbox workspace-write` 使用**: 子エージェントは worktree 内の sandbox で確認なしに操作する
